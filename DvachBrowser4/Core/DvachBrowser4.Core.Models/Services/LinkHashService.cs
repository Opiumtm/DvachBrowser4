...
namespace DvachBrowser4.Core.Models.Services
{
    ...
    /// Сервис хэша ссылок.
    /// </summary>
    internal sealed class LinkHashService : ServiceBase, ILinkHashService
    {
        private readonly Dictionary<Type, Func<BoardLinkBase, string>> _typeFuncs;

        /// <summary>
        /// Конструктор.
        /// </summary>
        public LinkHashService()
        {
            _typeFuncs = new Dictionary<Type, Func<BoardLinkBase, string>>()
            {
        ...
        public string GetLinkHash(BoardLinkBase link)
        {
            ...
            var t = link.GetType();
            if (_typeFuncs.ContainsKey(t))
            {
                return _typeFuncs[t](link).ToLowerInvariant();
            }
